...
class VisionLanguageModel(nn.Module):
    ...
    def __init__(self, cfg: VLMConfig, load_backbone: bool = True):
        ...
        if load_backbone:
        # SHAPE/ROLE: branche avec poids pretrained pour initialiser les backbones ViT et LM.
            self.vision_encoder = ViT.from_pretrained(cfg.vit)
            # SHAPE: vision_encoder: pixel_values [B, 3, 512, 512] -> image_feats [B, 1024, 768].
        else:
            self.vision_encoder = ViT(cfg.vit)
            # SHAPE: même interface que pretrained: [B, 3, 512, 512] -> [B, 1024, 768].

        if load_backbone:
        # SHAPE/ROLE: branche avec poids pretrained pour initialiser les backbones ViT et LM.
            self.decoder = LanguageModel.from_pretrained(self.cfg.lm)
            # SHAPE: decoder: embeddings [B, T, 960] -> hidden [B, T, 960].
        else:
            self.decoder = LanguageModel(self.cfg.lm)
            # SHAPE: même interface que pretrained: [B, T, 960] -> [B, T, 960].
        self.MP = ModalityProjector(cfg)              # the ModalityProjector
        # SHAPE: MP/projector: image_feats [B, 1024, 768] -> image_embedding [B, 64, 960].
        self.tokenizer = get_tokenizer(cfg.lm.tokenizer,cfg.image_token)        # the tokenizer (use get_tokenizer)
        # ROLE: tokenizer.image_token_id sert à trouver les positions <|image|> dans input_ids [B, T].
    ...
    def forward(self, input_ids, pixel_values, attention_mask=None, targets=None):
        ...
        # SHAPE: input_ids [B, T] -> token_embedding [B, T, 960].
        # ROLE: transforme les ids texte en vecteurs dans l'espace du LM; les <|image|> seront remplacés ensuite.

        #2
        images = self._process_images(pixel_values, input_ids.device)
        # SHAPE: pixel_values [B, 3, 512, 512] ou liste -> images [B, 3, 512, 512] sur le même device que input_ids.
        image_feats = self.vision_encoder(images)
        # SHAPE: images [B, 3, 512, 512] -> image_feats [B, 1024, 768].
        # ROLE: le ViT encode les 1024 patchs image en tokens visuels de dimension 768.

        #3
        image_embedding= self.MP(image_feats)
        # SHAPE: image_feats [B, 1024, 768] -> image_embedding [B, 64, 960].
        # ROLE: compresse les tokens ViT et les projette dans la dimension cachée du LM.
        
        #4
        token_embedding = self._replace_img_tokens_with_embd(input_ids, token_embedding, image_embedding)
        # SHAPE: token_embedding reste [B, T, 960].
        # ROLE: remplace les positions où input_ids == image_token_id par les 64 embeddings image.

        #5
        x,_ = self.decoder(token_embedding,attention_mask=attention_mask)
        # SHAPE: token_embedding [B, T, 960] -> x hidden [B, T, 960]; kv_cache ignoré ici.
        # ROLE: le LM traite la séquence fusionnée texte + image.
        if targets is None:
            return x,None
            # SHAPE: x [B, T, 960], pas de logits/loss si targets is None.
        else:
            logits = self.decoder.head(x)
            # SHAPE: x [B, T, 960] -> logits [B, T, vocab_size].
        
            shift_logits = logits[:, :-1, :].contiguous()   # positions 0..T-2 → prédisent 1..T-1
            # SHAPE: shift_logits [B, T-1, vocab_size].
            shift_labels = targets[:, 1:].contiguous()       # on jette le 1er label
            # SHAPE: shift_labels [B, T-1].
            loss = F.cross_entropy(
                shift_logits.view(-1, shift_logits.size(-1)),
                # SHAPE: [B*(T-1), vocab_size] pour CrossEntropy.
                shift_labels.view(-1),
                # SHAPE: [B*(T-1)].
                ignore_index=-100,
                # ROLE: ignore les positions masquées, notamment padding/prompt/tokens image selon targets.
            )
            return logits, loss
            # SHAPE: logits [B, T, vocab_size], loss scalaire.
